...
def loadData(filepath, P, Q, train_ratio, test_ratio, m_p_hour, log):
    # Traffic
    Traffic = np.load(filepath)['data']
    Traffic = np.expand_dims(Traffic,axis=-1) # (Data,Sensor) --> (Data, Sensor, 1)
    num_step = Traffic.shape[0]
    TE = np.zeros([num_step, 2])
    # m_p_hour * 24 = measure per day
    m_p_day = m_p_hour *24
    TE[:,1] = np.array([i % m_p_day for i in range(num_step)])
    TE[:,0] = np.array([(i // m_p_day) % 7 for i in range(num_step)])
