...
def construct_tem_adj(data, num_node, m_p_hour):
    data_mean = np.mean([data[24*m_p_hour*i: 24*m_p_hour*(i+1)] for i in range(data.shape[0]//(24*m_p_hour))], axis=0)
...
def loadGraph(spatial_graph, temporal_graph, dims, data, m_p_hour,log):
    ...
    if os.path.exists(temporal_graph):
        ...
        tem_adj = construct_tem_adj(data, adj.shape[0], m_p_hour)
