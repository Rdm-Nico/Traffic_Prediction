...
class Solver(object):
    ...
    def __init__(self, config):
        ...
        self.mean, self.std, data = loadData(
                                        ...
                                        self.train_ratio, self.test_ratio, self.m_p_hour, log)
        self.localadj, self.spawave, self.temwave = loadGraph(self.adj_file, self.tem_adj_file, self.heads*self.dims, data, self.m_p_hour, log)
        log_string(log, '------------ End -------------\n')

        self.best_epoch = 0

        self.device = torch.device(f"mps:{self.cuda}" if torch.backends.mps.is_available() else  "cpu")
        log_string(log, f'Using {self.device}')
        self.build_model()
        ...
        with torch.no_grad():
            for batch_idx in range(num_batch):
                if isinstance(self.model, torch.nn.Module):
                    ...
                    TE = torch.from_numpy(self.valTE[start_idx : end_idx,:,0,:]).float().to(self.device)
        ...
        for epoch in tqdm(range(1,self.max_epoch+1)):
            ...
            with tqdm(total=num_batch) as pbar:
                for batch_idx in range(num_batch):
                    ...
                    TE = torch.from_numpy(self.trainTE[start_idx : end_idx,:,0,:]).float().to(self.device)
        ...
        with torch.no_grad():
            for batch_idx in range(num_batch):
                if isinstance(self.model, torch.nn.Module):
                    ...
                    TE = torch.from_numpy(self.testTE[start_idx : end_idx,:,0,:]).float().to(self.device)
                    XL, XH = torch.from_numpy((XL-self.mean)/self.std).float().to(self.device), torch.from_numpy((XH-self.mean)/self.std).float().to(self.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--m_p_hour', type = int, default = config['data']['m_p_hour'])   
