...
def data_loader(X, Y, batch_size, device, shuffle=True, drop_last=True):
    # for the debugger  torch.mps.FloatTensor is not a attribute of torch.mps
    # cuda = True if torch.backends.mps.is_available() else False
...
def get_dataloader(args, normalizer = 'std', tod=False, dow=False, weather=False, single=True):
    ...
    ##############get dataloader######################
    train_dataloader = data_loader(x_train, y_train, args.batch_size, shuffle=True, drop_last=True, device=args.device)
    if len(x_val[..., 0]) == 0:
        val_dataloader = None
    else:
